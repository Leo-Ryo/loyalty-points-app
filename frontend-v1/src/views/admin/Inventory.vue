...
async function saveItem() {
  ...
  try {
    ...
    if (selectedFile.value) {
      // Use authenticated axios upload (not raw fetch which lacks the auth token)
      const res = await inventoryApi.uploadImage(selectedFile.value)
      imageUrl = res?.imageUrl ?? res?.data?.imageUrl
      if (!imageUrl) throw new Error('Upload succeeded but no imageUrl returned')
    }
    const data: any = {
      ...form,
      price: form.price ? parseFloat(form.price as any) : undefined,
      stockQuantity: parseInt(form.stockQuantity as any) || 0
    }
    if (imageUrl) data.imageUrl = imageUrl
    else if (editing.value?.imageUrl) data.imageUrl = editing.value.imageUrl // keep existing
    if (editing.value) { await inventoryApi.update(editing.value.id, data); notify.success('Item updated!') }
    else { await inventoryApi.create(data); notify.success('Item added!') }
    showModal.value = false; await load()
  } catch (e: any) { notify.error(e.message || 'Save failed') }
